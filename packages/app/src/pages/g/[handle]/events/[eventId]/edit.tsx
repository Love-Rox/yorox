      ...
      <form method="post" action={`/events/${eventId}/update`} className="mt-6 space-y-5">
        ...
        <label className="block">
          ...
          <input
            type="text"
            name="hashtags"
            defaultValue={(event.hashtags ?? []).map((t) => `#${t}`).join(' ')}
            className="input mt-1"
        ...
        <label className="block">
          <span className="text-sm font-bold">
            合計定員(任意)
            <HelpTip text="全枠あわせた上限です(会場キャパなど)。枠の定員に空きがあっても、合計がこの人数に達したら以降の申込は補欠になります。空欄で制限なし。" />
          </span>
          <input
            type="number"
            name="total_capacity"
            min={1}
            defaultValue={event.totalCapacity ?? undefined}
            className="input meta-mono mt-1"
            placeholder="例: 40"
          />
          <span className="mt-1 block text-sm text-neutral">
            補欠・抽選待ちは数に含めません(席が確定した人だけを数えます)
          </span>
        </label>
        ...
        <fieldset className="block">
          <legend className="text-sm font-bold">申込アンケート(イベント共通)<HelpTip text="どの枠から申し込んでも聞く質問です。枠だけの追加質問は各枠の編集画面で設定できます。回答は主催者だけが見られます。" /></legend>
          <p className="mt-1 text-sm text-neutral">
            申込時に聞く質問です。どの枠から申し込んでも表示されます。回答は主催者だけが確認できます。
          </p>
          <div className="mt-2">
            <SurveyBuilder name="survey_json" initial={event.applicationSurvey} />
          </div>
          <label className="mt-3 flex items-start gap-2">
            <input
              type="checkbox"
              name="survey_remote_policy"
              value="block"
              defaultChecked={event.surveyRemotePolicy === 'block'}
            />
            <span>
              必須アンケートがあるとき、Fediverse / Bluesky からのリモート申込を断る
              <span className="mt-1 block text-sm text-neutral">
                オフ(既定)なら、リモート申込は必須質問を免除して受け付けます
                (回答なしとして記録)。
              </span>
            </span>
          </label>
