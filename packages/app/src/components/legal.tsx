...
export async function instanceInfo() {
  ...
  return {
    ...
    contact: env.LEGAL_CONTACT ?? '(連絡先を設定してください)',
    configured: !!(env.LEGAL_OPERATOR && env.LEGAL_CONTACT),
  };
}
...
export function OperatorNotice({ configured }: { configured: boolean }) {
  ...
  return (
    <p className="mt-4 border-2 border-accent p-3 text-sm text-accent">
      ...
      運営者情報(<span className="meta-mono">INSTANCE_NAME / LEGAL_OPERATOR / LEGAL_CONTACT</span>)
      を設定してください。
    </p>
  );
}
